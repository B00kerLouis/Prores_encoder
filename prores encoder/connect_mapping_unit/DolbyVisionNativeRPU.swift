...
private struct NativeDolbyVisionRPUBuilder {
    ...
    private func writeHeader(to writer: inout NativeDolbyVisionBitWriter) {
        ...
        writer.writeBit(profile.isProfile76)
        writer.writeBit(!profile.isProfile76)
        writer.writeBit(true)
        writer.writeBit(false)
    }
    ...
    private func writeMapping(to writer: inout NativeDolbyVisionBitWriter) {
        ...
        } else if profile.usesProfile84Mapping {
    ...
    private func writeProfile76Mapping(to writer: inout NativeDolbyVisionBitWriter) {
        ...
        for _ in 0..<3 {
            writer.writeUE(0) // num_pivots_minus2
            writer.write(0, bits: 10)
            writer.write(1023, bits: 10)
        }

        writer.write(0, bits: 3) // nlq_method_idc: linear dead-zone
        writer.write(0, bits: 10)
        writer.write(1023, bits: 10)
        ...
        // FEL NLQ parameters for a 10-bit residual centered on code value 512.
        for _ in 0..<3 {
            writer.write(512, bits: 10)       // nlq_offset
            writer.writeUE(0)                 // vdr_in_max_int
            writer.write(1_048_576, bits: 23) // vdr_in_max
            writer.writeUE(0)                 // linear_deadzone_slope_int
            writer.write(2_048, bits: 23)     // linear_deadzone_slope
            writer.writeUE(0)                 // linear_deadzone_threshold_int
            writer.write(0, bits: 23)         // linear_deadzone_threshold
        }
    }
